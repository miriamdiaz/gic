...
<html>
<head>
<title>Busqueda de Lugar de Edicion</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<script language="JavaScript">
function agregar(valor)
{
var input=window.opener.document.getElementById("lugar_edicion");
if(input.value!="")
...
<table class="primary">
  <tr>
    <th nowrap="yes" align="left"> Busque un Lugar de Edicion</th>
  </tr>
<tr class="primary">
<form name="search" action="search_lugar_edicion.php">  
    <td class="primary">
...
$db=new Query();
$db->connect();
$db->_query("delete from lugar_edicion","");
if(isset($_GET["valor"]))
{
$valor=$_GET["valor"];
$sql="SELECT field_data ";
$sql.="FROM biblio_field ";
$sql.="WHERE tag=260 AND subfield_cd = 'a' ";
...
while($row=mysql_fetch_array($result))
    {
	 ...
	 foreach($field_data_array as $field_data_row)
	       {
			$sql="insert into lugar_edicion values ('$field_data_row')";
			$db->_query($sql,"");
		   }
	}
$sql="select lugar_edicion from lugar_edicion where lugar_edicion like '".$valor."%' group by lugar_edicion order by lugar_edicion";
$result=$db->_query($sql,"");
