 ...
 */
  ...
  //echo"<h1>".$_POST["opacFlg"]."</h1>";
  //echo"<h1> lite ".$_POST["literaturaFlg"]."</h1>";
  $biblio->setOpacFlg(isset($_POST["opacFlg"]));
    //echo"<h1>".$biblio->showInOpac()."</h1>";
  $biblio->setLiteraturaFlg(isset($_POST["literaturaFlg"]));  
     // echo"<h1> lite ".$biblio->getLiteraturaFlg()."</h1>";
  /*ini fran*/
  $dia=$HTTP_POST_VARS["list_day"];
  $mes=$HTTP_POST_VARS["list_month"];
  $anio=$HTTP_POST_VARS["list_year"];
  $biblio->setUserNameCreador($_SESSION["userid"]);
  $postVars["user_name_creador"]= $biblio->getUserNameCreador();
  $biblio->setFechaCatalog($anio."-".$mes."-".$dia);
  $biblio->setIndice($_POST["indice"]);
  $postVars["indice"]= $biblio->getIndice();
  ...
  if (!$validData) {
	...
	//$pageErrors["fecha"]="Fecha Seleccionada Incorrecta"." ".$dia."/".$mes."/".$anio;
    /*fin franco*/
  ...
  if (!$bibid = $biblioQ->insert($biblio)) 
  {
	...
	if ($biblioQ->getDbErrno() == "") 
	{
      ...
      $_SESSION["postVars"] = $_POST;
      $_SESSION["pageErrors"] = $pageErrors;
      header("Location: ../catalog/biblio_new_form.php?bibid=".$bibid);
	  exit();
    }
	else
	{
