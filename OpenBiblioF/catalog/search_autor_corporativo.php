...
<html>
<head>
<title>Busqueda de Autor Corporativo</title>
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<script language="JavaScript">
function agregar(valor)
{
var input=window.opener.document.getElementById("autor_corporativo");
if(input.value!="")
...
<table class="primary">
  <tr>
    <th nowrap="yes" align="left"> Busque un Autor Corporativo</th>
  </tr>
<tr class="primary">
<form name="search" action="search_autor_corporativo.php">  
    <td class="primary">
...
$db=new Query();
$db->connect();
$db->_query("delete from autor_corporativo","");
if(isset($_GET["valor"]))
{
$valor=$_GET["valor"];
$sql="SELECT field_data ";
$sql.="FROM biblio_field ";
$sql.="WHERE tag=110 AND subfield_cd = 'a' ";
...
while($row=mysql_fetch_array($result))
    {
	 ...
	 foreach($field_data_array as $field_data_row)
	       {
			$sql="insert into autor_corporativo values ('$field_data_row')";
			$db->_query($sql,"");
		   }
	}
$sql="select autor_corporativo from autor_corporativo where autor_corporativo like '".$valor."%' group by autor_corporativo order by autor_corporativo";
$result=$db->_query($sql,"");	
if(mysql_num_rows($result)>0)
  {
   ...
   while($row=mysql_fetch_array($result))
        {
         $field_data=$row["autor_corporativo"];
         ?>
          <tr class="primary">
             <td class="primary">
	             <? echo $field_data;?>
