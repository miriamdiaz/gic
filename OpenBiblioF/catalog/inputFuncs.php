...
Fecha: 28-06-06
Descripcion: Se agregó esta función para incluir 
el boton de buscador de valores posibles para campos.
*/

function printUsmarcInputTextWithSearch($tag,$subfieldCd,$required,&$fieldIds,&$postVars,&$pageErrors,&$marcTags,&$marcSubflds,$showTagDesc,$cntrlType,$occur="",$input_id=""){
  $arrayIndex = sprintf("%03d",$tag).$subfieldCd;
  $formIndex = $arrayIndex.$occur;
  $size = 40;
  $maxLen = 3000;
  ...
  if ($cntrlType == OBIB_TEXTAREA_CNTRL) {
    ...
    echo "<input type=\"text\"";
    echo "\" name=\"values[".$formIndex."]\" id=\"".$input_id."\" size=\"".$size."\" maxlength=\"".$maxLen."\" ";
    echo "value=\"".$value."\" >";
  }
  if ($error != "") {
    echo "<br><font class=\"error\">";
