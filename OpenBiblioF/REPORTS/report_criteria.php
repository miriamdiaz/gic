 ...
 */
  ...
  function printCriteriaFields($index,&$fieldIds,&$fieldNames,&$fieldTypes,&$fieldNumericFlgs,&$postVars,&$pageErrors,&$loc,&$fieldValuebVisibility){
	...
	echo "<td class='primary' align='left' valign='top' nowrap='yes'>";
    echo "<select name=\"".$fldIndex."\" onchange=\"fieldIndexOnChange(this,".$index.")\">";
    echo "<option value=\"\"";
    ...
    foreach($fieldIds as $fldid) {
      ...
      if ((isset($postVars[$fldIndex])) and ($postVars[$fldIndex] == $fld)) 
	     {
	     echo " selected";
		 $selected = $fld;
		 }
    ...
    echo "</select> ";
	echo "</td>";
	echo "<td id='fieldValue".$index."' class='primary' align='left' valign='top' nowrap='yes'>";
	if($selected == "concepto_dm.description string 0")
	  {
	     $reference = "";
		 printSelectReferenceSinCode("fieldValue".$index."a","concepto_dm",$postVars,$reference);
		 echo $reference;
	...
	elseif($selected == "area_dm.description string 0")
	  {
	     $reference = "";
		 printSelectReferenceSinCode("fieldValue".$index."a","area_dm",$postVars,$reference);
		 echo $reference;
		 echo"<span id=\"and".$index."\" style=\"visibility:".$fieldValuebVisibility[$index].";\"> ".$loc->getText("reportCriteriaAnd")." </span>";
		 printInputText("fieldValue".$index."b",15,80,$postVars,$pageErrors,$fieldValuebVisibility[$index]);
	  }	  
	elseif($selected == "biblio_cod_library.description string 0")
	  {
	     $reference = "";
		 printSelectReferenceSinCode("fieldValue".$index."a","biblio_cod_library",$postVars,$reference);
		 echo $reference;
		 echo"<span id=\"and".$index."\" style=\"visibility:".$fieldValuebVisibility[$index].";\"> ".$loc->getText("reportCriteriaAnd")." </span>";
		 printInputText("fieldValue".$index."b",15,80,$postVars,$pageErrors,$fieldValuebVisibility[$index]);
	  }
	elseif($selected == "estado_dm.description string 0")
	  {
	     $reference = "";
		 printSelectReferenceSinCode("fieldValue".$index."a","estado_dm",$postVars,$reference);
		 echo $reference;
		 echo"<span id=\"and".$index."\" style=\"visibility:".$fieldValuebVisibility[$index].";\"> ".$loc->getText("reportCriteriaAnd")." </span>";
		 printInputText("fieldValue".$index."b",15,80,$postVars,$pageErrors,$fieldValuebVisibility[$index]);
	  }	  	  	  
  ...
  if($rptid == "adquisiciones")
     $sql = stripslashes($_SESSION["sqlAdquisiciones"]);
  else
...
?>
  ...
  function fieldIndexOnChange(inputElem,critNmbr) {
    var option = inputElem[inputElem.selectedIndex].value;
	var input = 'fieldValue'+critNmbr;
	var element = document.getElementById(input);
	if(option == "concepto_dm.description string 0")
	  {
       var contenido = "<select id='"+input+"a' name='"+input+"a' > ";
	   contenido += document.reportcriteriaform.conceptos.value;
	   contenido += "</select>";
	   contenido += "<span id=and"+critNmbr+" style='VISIBILITY:hidden'>y</span>";
       contenido +="<select id='"+input+"b' name='"+input+"b' style='VISIBILITY:hidden'> ";
	   contenido += document.reportcriteriaform.conceptos.value;
	   contenido += "</select>";	   
	   element.innerHTML = contenido;
	  }
	else if(option == "area_dm.description string 0")
	  {
       var contenido = "<select id='"+input+"a' name='"+input+"a' > ";
	   contenido += document.reportcriteriaform.areas.value;
	   contenido += "</select>";
	   contenido += "<span id=and"+critNmbr+" style='VISIBILITY:hidden'>y</span>";
       contenido +="<select id='"+input+"b' name='"+input+"b' style='VISIBILITY:hidden'> ";
	   contenido += document.reportcriteriaform.conceptos.value;
	...
	else if(option == "biblio_cod_library.description string 0")
	  {
       var contenido = "<select id='"+input+"a' name='"+input+"a' > ";
	   contenido += document.reportcriteriaform.bibliotecas.value;
	   contenido += "</select>";
	   contenido += "<span id=and"+critNmbr+" style='VISIBILITY:hidden'>y</span>";
       contenido +="<select id='"+input+"b' name='"+input+"b' style='VISIBILITY:hidden'> ";
	   contenido += document.reportcriteriaform.conceptos.value;
	...
	else if(option == "estado_dm.description string 0")
	  {
       var contenido = "<select id='"+input+"a' name='"+input+"a' > ";
	   contenido += document.reportcriteriaform.estados.value;
	   contenido += "</select>";
	   contenido += "<span id=and"+critNmbr+" style='VISIBILITY:hidden'>y</span>";
       contenido +="<select id='"+input+"b' name='"+input+"b' style='VISIBILITY:hidden'> ";
	   contenido += document.reportcriteriaform.conceptos.value;
	  ...
	  {
	   var contenido="<input type=text name="+input+"a size=15 maxlength=80>";
	   contenido += "<span id=and"+critNmbr+" style='VISIBILITY:hidden'>y</span>";
	   contenido += "<input type=text name="+input+"b size=15 maxlength=80 style='VISIBILITY:hidden'>";
	   element.innerHTML = contenido;
	  }   
  }  
...
<?php 
$reference="";
printSelectReferenceSinCodeSoloOptions("concepto_dm",$postVars,$reference);
?>
<input type="hidden" name="conceptos" value="<?=$reference?>">

<?php 
$reference="";
printSelectReferenceSinCodeSoloOptions("area_dm",$postVars,$reference);
?>
<input type="hidden" name="areas" value="<?=$reference?>">

<?php 
$reference="";
printSelectReferenceSinCodeSoloOptions("biblio_cod_library",$postVars,$reference);
?>
<input type="hidden" name="bibliotecas" value="<?=$reference?>">

<?php 
$reference="";
printSelectReferenceSinCodeSoloOptions("estado_dm",$postVars,$reference);
?>
<input type="hidden" name="estados" value="<?=$reference?>">
...
?>
  ...
  <input type="hidden" name="rptid" value="<?php echo $rptid_aux;?>">
  <input type="hidden" name="title" value="<?php echo $title_aux;?>">
  <input type="hidden" name="sql" value="<?php echo $sql_aux;?>">
