 ...
 */
  ...
  if (isset($_GET["mbrid"])){
	...
	$postVars["libraryid"] = $mbr->getLibraryid();
	$postVars["limite_prestamos"] = $mbr->getLimitePrestamos();
	//DOS LINEAS AGREGADAS: Horacio Alvarez FECHA: 08-04-06
	$postVars["limite_reservas"] = $mbr->getLimiteReservas();	
	...
	$postVars["observaciones"] = $mbr->getObservaciones();
    $postVars["schoolTeacher"] = $mbr->getSchoolTeacher();
	$postVars["cantidad_reservas"] = $mbr->getCantidadReservas();
	$postVars["cantidad_prestamos"] = $mbr->getCantidadPrestamos();
	$postVars["tipo_sancion_cd"] = $mbr->getTipo_sancion_cd();
	$postVars["inicio_sancion"] = $mbr->getInicio_sancion();
	$postVars["fecha_suspension"] = $mbr->getFecha_suspension();
	$postVars["sancion_activa"] = $mbr->getSancion_activa();
	$postVars["copy_barcode"] = $mbr->getCopy_barcode();
	$postVars["fechavto"] = $mbr->getFechaVto();
	$postVars["carrera"] = $mbr->getCarrera();
  ...
  $cancelLocation = "../adquisiciones/mbr_list.php?mbrid=".$postVars["mbrid"];
?>

<form name="editMbrform" method="POST" action="../adquisiciones/mbr_edit.php">
<input type="hidden" name="mbrid" value="<?php echo $postVars["mbrid"];?>">
<?php include("../adquisiciones/mbr_fields.php"); ?>
