 ...
 */
  ...
  $loc = new Localize("es",$tab);
  Header("Content-type: application/vnd.ms-excel; charset=".OBIB_CHARSET.";");
  Header("Content-disposition: attachment; filename=busqueda_avanzada.csv");
  include("../reports/run_report.php");



echo "Resultado de la busqueda Avanzada: \n\n";
...
<?php
    while ($array = $reportQ->fetchRow()) {
        foreach($array as $key=>$cell) {
		    if(substr_count($fieldIds[$key],"topic")>0)
