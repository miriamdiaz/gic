...
<table class="primary">
  ...
  <tr>	
    ...
    <td valign="top" class="primary">
      <?php echo $materialTypeDm[$biblio->getMaterialCd()]." - Bibid: ".$biblio->getBibid();?>
    </td>
  </tr>
  <tr>
    <td nowrap="true" class="primary" valign="top">
 ...
 <?php }?>
  ...
  <tr>
	<td class="primary" valign="top">
		<?php echo $loc->getText("biblioImagen"); ?>:
	</td>
	<!--
	Autor: Horacio Alvarez
	Fecha: 27-05-06
	Descripcion: Lugar donde se muestra la tapa del material.
	Se valida que exista y que no hallan errores al cargarse la imagen desde submit_foto.php
	--->
	<td valign="top" align="center" class="primary">
     <img src="../fotos/foto.php?bibid=<?=$bibid?>" width="70" height="90">
	</td>
  </tr>
</table>
<br>
...
<?php if ($tab == "cataloging") { ?>
	<br>
	<form action="submit_foto.php" method="post" enctype="multipart/form-data"> 
	<?php echo $loc->getText("biblioUploadImage"); ?>:
	<input name="userfile" type="file">
	<input type="hidden" name="MAX_FILE_SIZE" value="100000"> 
	<input type="hidden" name="bibid" value="<?=$_GET["bibid"];?>">
	<input type="submit" value="<?php print $loc->getText("biblioUpload"); ?>" class="button">
	</form>
<?php } ?>
<!-- /\ Eduardo ROJEL 20/12/05 /\ -->
<br>
...
<table class="primary">
  <tr>
	...
	<th align="left" nowrap="yes">
      <?php echo $loc->getText("biblioViewTble4Col5"); ?>
    </th>
    <th align="left" nowrap="yes">
  ...
  ?>
    <tr>
	  ...
	  <td valign="top" class="<?php echo $row_class;?>">
        <?php echo $ana->getAnaliticaSubTitulo(); ?>
      </td>
      <td valign="top" class="<?php echo $row_class;?>">
...
<?php
  ...
  if ($tab == "cataloging") { ?>
    ...
    <?php
    $copyCols=9;//antes 12 antes 7
  } else {
    $copyCols=7;//antes 10 antes 5
  }
...
<table class="primary">
  <tr>
    <?php 
	$colspan = 2; 
	if($biblio->getMaterialCd() == 6)
	   $colspan = 3; 	
	if ($tab == "cataloging") { ?>
      <th colspan="<?=$colspan?>" nowrap="yes">
        <?php echo $loc->getText("biblioViewTble2ColFunc"); ?>
      </th>
    <?php } ?>
    <th align="left" nowrap="yes">
    ...
    <tr>
      <?php if ($tab == "cataloging") { ?>
	    ...
	    if($biblio->getMaterialCd() == 6)
	      { ?>
          <td valign="top" class="<?php echo $row_class;?>">
             <a href="../catalog/biblio_copy_num_list.php?bibid=<?php echo $copy->getBibid();?>&copyid=<?php echo $copy->getCopyid();?>" class="<?php echo $row_class;?>"><?php echo "Estado de Colección"; ?></a>
          </td>		  
		<? }
		?>
      <?php } ?>
