 ...
 ******************************************************************************
 */
class ReportBibliosQuery extends Query {
  var $_rowCount = 0;
  var $_loc;
  var $_itemsPerPage = 1;
  var $_rowNmbr = 0;
 ...
 function viewBiblios($page,$fecha1,$fecha2)
	{
    	...
    	$sqlcount = "select count(*) as rowcount " 
		           . "from biblio " 
				   . "where biblio.create_dt >= '$fecha1' "
                   . "and biblio.create_dt <= '$fecha2' ";
	    ...
	    $sql = $this->mkSQL("select biblio.* "
		                  . "from biblio "
						  . "where biblio.create_dt >= %Q "
						  . "and biblio.create_dt <= %Q ",
                               $fecha1, $fecha2); 
 ...
 function viewPrestamos($page,$fecha1,$fecha2,$materialCd)
	{
    	...
    	$sqlcount = "select count(*) as rowcount "
                    ...
                    ." left join biblio C on C.bibid=H.bibid "
                    ." left join member M on H.mbrid=M.mbrid "
                    ." where H.status_begin_dt between '$fecha1' and '$fecha2' "
                    ." and H.status_cd like 'out' "
                    ." and ( C.material_cd = $materialCd ) ";
		//echo "sql Count: $sqlcount";
	    ...
	    $sql = $this->mkSQL("select * "
                            ...
                            ." left join biblio C on C.bibid=H.bibid "
                            ." left join member M on H.mbrid=M.mbrid "
                            ." where H.status_begin_dt between %Q and %Q "
                            ." and H.status_cd like 'out' "
                            ." and ( C.material_cd = $materialCd ) "
  ...
  function fetchRowMaterialesPrestados()
  {
    $array = $this->_conn->fetchRow();
    if ($array == false) 
    {
    ...
    $bib = new Biblio();
    $bib->setBibid($array["bibid"]);
	$bib->setCreateDt($array["status_begin_dt"]);
	$bib->setMaterialCd($array["material_cd"]);
	$bib->setTitle($array["title"]);
	$bib->setCallNmbr1($array["call_nmbr1"]);
	$bib->setCallNmbr2($array["call_nmbr2"]);
