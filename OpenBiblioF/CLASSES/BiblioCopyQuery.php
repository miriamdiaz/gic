...
//fin franco
  ...
  function execSelect($bibid) {
    ...
    $sql = $this->mkSQL("select biblio_copy.* "
                        ...
                        $bibid);
    $sql.=" ORDER BY biblio_copy.barcode_nmbr ";
    if (!$this->_query($sql, $this->_loc->getText("biblioCopyQueryErr4"))) {
      return false;
    }
    $this->_rowCount = $this->_conn->numRows();
  ...
  function insert($copy) {
	...
	/*******  comentado por pedido de SPTU 17-05-06
	# checking for duplicate %-barcode number 21/02/06
    $dupBarcodeSeparate = $this->_dupBarcodeSeparate($copy->getBarcodeNmbr());
    if ($this->errorOccurred()) return false;
    if ($dupBarcodeSeparate) {
      $this->_errorOccurred = true;
	  $barcode_separate = explode ("-", $copy->getBarcodeNmbr());
      $this->_error = $this->_loc->getText("biblioCopyQueryErr11",array("barcodeNmbr_sep"=>$barcode_separate[1]));
      return false;
    }
	*********/
