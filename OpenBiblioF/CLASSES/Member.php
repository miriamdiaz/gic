...
class Member {
  ...
  function validateData() {
    ...
    if ($this->_barcodeNmbr == "") {
      $valid = false;
      $this->_barcodeNmbrError = "El número del socio es requerido.";
    } else if (!ctypeAlnum($this->_barcodeNmbr)) {
      $valid = FALSE;
      $this->_barcodeNmbrError = "El número del socio debe ser caracteres alfabéticos y numéricos.";
    }
    if ($this->_lastName == "") {
      $valid = false;
	...
	//IF AGREGADO: Horacio Alvarez FECHA: 26-06-06
    if ($this->_classification == "") {
      $valid = false;
      $this->_classificationError = "La clasificación del socio es requerida.";
    }	
	//IF AGREGADO: Horacio Alvarez FECHA: 24-03-06
    if ($this->_libraryid == "") {
      $valid = false;
    ...
    if (!is_numeric($this->_limitePrestamos)) {
      $valid = false;
      $this->_limitePrestamosError = "El limite de préstamos debe ser numérico.";
    }
	//IF AGREGADO: Horacio Alvarez FECHA: 08-04-06
    if (!is_numeric($this->_limiteReservas)) {
      $valid = false;
      $this->_limiteReservasError = "El limite de reservas debe ser numérico.";
    }	
    if (!is_numeric($this->_zip)) {
      $valid = false;
      $this->_zipError = "El código postal debe ser numérico.";
    }
    if (strrpos($this->_zip,".")) {
      $valid = false;
      $this->_zipError = "El código postal no debe contener un punto decimal.";
    }
    if (!is_numeric($this->_zipExt)) {
      $valid = false;
      $this->_zipExtError = "La extensión del código postal debe ser numérico.";
    }
    if (strrpos($this->_zipExt,".")) {
      $valid = false;
      $this->_zipExtError = "La extensión del código postal no debe contener un punto decimal.";
    }
/*    if (!is_numeric($this->_schoolGrade)) {
      $valid = false;
      $this->_schoolGradeError = "El grado de escuela debe ser numérico.";
    }*/
  ...
  function getFechaVto() {
    return $this->_fechaVto;
  }
  function setFechaVto($value) {
  ...
  */
  ...
  Fecha: 08-04-06
  */
  function getClassificationError() {
    return $this->_classificationError;
  }  
  /**
  Autor: Horacio Alvarez
  Fecha: 08-04-06
  ...
  */
  ...
  Autor: Horacio Alvarez
  Fecha: 20-05-06
  Descripcion: Devuelve la cantidad de dias de suspensión.
  */   
  function getCantidadDiasSuspension() {
    $inicio=$this->getInicio_sancion();
//	echo "this->getInicio_sancion: ".$this->getInicio_sancion()."<br>";
	$final=$this->getFecha_suspension();
//	echo "this->getFecha_suspension: ".$this->getFecha_suspension()."<br>";
    $date1 = mktime(0,0,0,substr($inicio,5,2),substr($inicio,8,2),substr($inicio, 0,4));
    $date2 = mktime(0,0,0,substr($final,5,2),substr($final,8,2),substr($final, 0,4));
    $total_days = Round((($date2-$date1)/86400), 0) ; 
	
	if($this->getTipo_sancion_cd()==1)
	   $total_days = 0;
	return $total_days;
  ...
  function setLastName($value) {
    $this->_lastName = strtoupper(trim($value));
  }
  ...
  */
  ...
  function setObservaciones($value) {
    $this->_observaciones = trim($value);
  }  
  ...
  function setCarrera($value) {
    $this->_carrera = trim($value);
  }      
