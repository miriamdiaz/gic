 ...
 ******************************************************************************
 */
class ReportAprobCopyQuery extends Query {
  var $_rowCount = 0;
  var $_loc;
...
//fin franco
  ...
  function hasReachedCheckoutLimit($mbrid,$classification,$bibid) {
	...
	function viewCopyAprob($page,$fecha1,$fecha2)
	{
		...
		$sqlcount = "SELECT count(*) AS rowcount "
					."FROM biblio_copy, auditoria, biblio "
					."WHERE biblio_copy.aprob_flg = 1 "
					."AND biblio_copy.bibid = auditoria.bibid "
					."AND biblio_copy.bibid = biblio.bibid "
					."AND auditoria.nombretabla = \"biblio_copy\" "
					."AND auditoria.operacion = 4 "
					."AND auditoria.fecha >= '$fecha1' AND auditoria.fecha <= '$fecha2' ";
		
		
		# setting query that will return all the data
		$sql = $this->mkSQL("SELECT biblio_copy.bibid, biblio_copy.copyid, "
							."biblio_copy.barcode_nmbr, biblio.title, "
							."biblio_copy.status_begin_dt, biblio_copy.copy_precio, "
							."biblio_copy.copy_cod_loc, biblio_copy.copy_user_creador, "
							."auditoria.userid, auditoria.fecha "
							."FROM biblio, biblio_copy, auditoria "
							."WHERE biblio_copy.aprob_flg = 1 "
							."AND biblio_copy.bibid = auditoria.bibid "
							."AND biblio_copy.bibid = biblio.bibid "
							."AND auditoria.nombretabla = %Q "
						    ."AND auditoria.operacion = %N "
						    ."AND auditoria.fecha >= %Q "
						    ."AND auditoria.fecha <= %Q ",
                              "biblio_copy", 4, $fecha1, $fecha2);
