...
class SettingsQuery extends Query {
  ...
  Fecha: 17-04-06
  Modificacion: Se modifica para agregar los campos unidad academica y domicilio.
  */
  function fetchRow() {
	...
	$set->setLibraryCd($array["library_cd"]);
	...
	$set->setUnidad_academica($array["unidad_academica"]);
	$set->setDomicilio($array["domicilio"]);
	$set->setSmtp($array["smtp"]);
	$set->setImap($array["imap"]);
    if ($array["usuarios_online_flg"] == 'Y') {
      $set->setUsuariosOnlineFlg(true);
    } else {
      $set->setUsuariosOnlineFlg(false);
    }
    if ($array["docentes_online_flg"] == 'Y') {
      $set->setDocentesOnlineFlg(true);
    } else {
      $set->setDocentesOnlineFlg(false);
    }		
  ...
  */
  function update($set) {
    $sql = $this->mkSQL("update settings set "
                        . "library_name=%Q, library_cd=%Q, library_image_url=%Q, "
						...
						. "unidad_academica=%Q, domicilio=%Q , smtp=%Q , imap=%Q, usuarios_online_flg=%Q, docentes_online_flg=%Q ",
                        $set->getLibraryName(), $set->getLibraryCd(), 
                        ...
                        $set->getHtmlLangAttr(),$set->getUnidad_academica(),$set->getDomicilio(),$set->getSmtp(),$set->getImap(),
						$set->getUsuariosOnlineFlg() ? "Y" : "N",$set->getDocentesOnlineFlg() ? "Y" : "N");
