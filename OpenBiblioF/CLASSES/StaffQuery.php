...
// . " and pwd <> 1 ",             
    ...
    return $this->_query($sql, "Error al verificar nombre de usuario y contraseña.");
  }
  ...
  function suspendStaff($username)
  {
    ...
    return $this->_query($sql, "Error al suspender un miembro del staff.");
  }
  ...
  */   
  function fetchStaff() {
	...
	if ($array["limpiar_sancion_flg"] == "Y") {
      $staff->setLimpiarSancion(TRUE);
    } else {
      $staff->setLimpiarSancion(FALSE);
    }
	
	if ($array["admin_bibliotecarios_flg"] == "Y") {
      $staff->setAdminBiblioAuth(TRUE);
    } else {
      $staff->setAdminBiblioAuth(FALSE);
    }		
    ...
    if ($array["exportar_flg"] == "Y") {
      $staff->setExportarAuth(true);
    } else {
      $staff->setExportarAuth(false);
    }
	
    if ($array["importar_flg"] == "Y") {
      $staff->setImportarAuth(true);
    } else {
      $staff->setImportarAuth(false);
    }	
	
    if ($array["adquisicion_flg"] == "Y") {
      $staff->setAdquisicionesAuth(true);
    } else {
      $staff->setAdquisicionesAuth(false);
    }			
    return $staff;
  }
   ...
   */
   ...
   Autor: Horacio Alvarez
   Fecha: 05-07-2006
   Descripcion: Se saco un echo $sql
   */
  function insert($staff) {
    ...
    if ($dupUsername) {
      ...
      $this->_error = "El nombre de usuario ya está en uso.";
      return false;
    }
    ...
    $sql .= $this->mkSQL("'N', %Q, %Q, %Q, %Q, %Q, %Q, %Q, %Q, %Q, %Q, %Q) ",
						 ...
						 $staff->hasLimpiarSancion() ? "Y" : "N",
						 $staff->hasAdminBiblioAuth() ? "Y" : "N",
						 $staff->hasImportarAuth() ? "Y" : "N",
						 $staff->hasExportarAuth() ? "Y" : "N",
						 $staff->hasAdquisicionesAuth() ? "Y" : "N");
   
    return $this->_query($sql, "Error al insertar la información de un nuevo miembro del staff.");
  }
  ...
  */
  function update($staff) {
    ...
    $sql .= $this->mkSQL("suspended_flg=%Q, admin_flg=%Q, circ_flg=%Q, "
                         . "circ_mbr_flg=%Q, catalog_flg=%Q, reports_flg=%Q, aprobar_flg=%Q, limpiar_sancion_flg=%Q, admin_bibliotecarios_flg=%Q, importar_flg=%Q, exportar_flg=%Q, adquisicion_flg = %Q "
                         . "where userid=%N ",
    ...
    return $this->_query($sql, "Error al actualizar la información de un miembro del staff.");
  }
  ...
  function resetPwd($staff) {
    ...
    return $this->_query($sql, "Error al resetear el  password.");
  }
  ...
  function delete($userid) {
    ...
    return $this->_query($sql, "Error al borrar informacion de un miembro del staff.");
  }
