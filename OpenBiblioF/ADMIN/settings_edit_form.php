 ...
 */
  ...
  if (isset($_GET["reset"])){
	...
	//dos linas agregadas: Horacio Alvarez Fecha: 2006-04-17
	$postVars["unidad_academica"] = $set->getUnidad_academica();
	$postVars["domicilio"] = $set->getDomicilio();
	$postVars["library_cd"] = $set->getLibraryCd();
	$postVars["smtp"] = $set->getSmtp();
	$postVars["imap"] = $set->getImap();
	$postVars["usuarios_online_flg"] = $set->getUsuariosOnlineFlg();
	$postVars["docentes_online_flg"] = $set->getDocentesOnlineFlg();
    $setQ->close();
  } else {
...
<form name="editsettingsform" method="POST" action="../admin/settings_edit.php" enctype="multipart/form-data">
...
<table class="primary">
  ...
  Fecha: 17-04-06
  Descripcion: se agrega el campo Unidad Academica
  -->
  <tr>
    <td nowrap="true" class="primary">
      <? echo $loc->getText("admin_settingsUnidad_academica"); ?>
    </td>
    <td valign="top" class="primary">
      <?php printInputText("unidad_academica",40,128,$postVars,$pageErrors); ?>
    </td>
  </tr>  
  <!--
  Autor: Horacio Alvarez
  Fecha: 25-07-06
  Descripcion: se agrega el campo Codigo de Biblioteca
  -->
  <tr>
    <td nowrap="true" class="primary">
      <? echo $loc->getText("admin_settingsCodigoBiblioteca"); ?>
    </td>
    <td valign="top" class="primary">
      <?php //printInputText("library_cd",40,128,$postVars,$pageErrors); ?>
	  <?php printCheckBoxs("biblio_cod_library","library_cd",$postVars,$pageErrors); ?>
    </td>
  </tr>    
  <!--
  Autor: Horacio Alvarez
  Fecha: 17-04-06
  Descripcion: se agrega el campo domicilio.
  -->
  <tr>
    <td nowrap="true" class="primary">
      <? echo $loc->getText("admin_settingsDomicilio"); ?>
    </td>
    <td valign="top" class="primary">
      <?php printInputText("domicilio",40,128,$postVars,$pageErrors); ?>
    </td>
  </tr>    
  <tr>
    <td nowrap="true" class="primary">
  ...
  <tr>
    ...
    <td valign="top" class="primary">
	    <?php printInputText("smtp",30,50,$postVars,$pageErrors); ?>
    </td>
  </tr>
  <tr>
    <td nowrap="true" class="primary">
    ...
    <td valign="top" class="primary">
	   <?php printInputText("imap",30,50,$postVars,$pageErrors); ?>
    </td>
  </tr>
  <tr>
    <td nowrap="true" class="primary">
      <? echo "Link Usuarios Online"; ?>
    </td>
    <td valign="top" class="primary">
      <input type="checkbox" name="usuarios_online_flg" value="CHECKED"
        <?php if ($postVars["usuarios_online_flg"] == "Y") echo "checked"; ?> >
    </td>
  </tr>  
  <tr>
    <td nowrap="true" class="primary">
      <? echo "Link Docentes Online"; ?>
    </td>
    <td valign="top" class="primary">
      <input type="checkbox" name="docentes_online_flg" value="CHECKED"
        <?php if ($postVars["docentes_online_flg"] == "Y") echo "checked"; ?> >
    </td>
  </tr>    
  <tr>
