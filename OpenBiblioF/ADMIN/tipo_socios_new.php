 ...
 */
  ...
  if (count($_POST) == 0) {
    header("Location: ../admin/tipo_socios_new_form.php");
    exit();
  }
...
/*  $dmQ = new DmQuery();
  ...
  if (!$dmQ->insert("mbr_classify_dm",$dm)) {
    $dmQ->close();
    displayErrorPage($dmQ);
  }
  $dmQ->close();
...
?>
<? echo $loc->getText("adminTipoSocios_delStart"); ?><?php echo $dm->getDescription();?><? echo $loc->getText("adminTipoSocios_newAdded"); ?><br><br>
