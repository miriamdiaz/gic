 ...
 */
  ...
  if (count($_POST) == 0) {
    header("Location: ../admin/tipo_sanciones_new_form.php");
    exit();
  }
...
/*  $dmQ = new DmQuery();
  ...
  if (!$dmQ->insert("tipo_sancion_dm",$dm)) {
    $dmQ->close();
    displayErrorPage($dmQ);
  }
  $dmQ->close();
...
?>
<? echo $loc->getText("adminTipoSanciones_delStart"); ?><?php echo $dm->getDescription();?><? echo $loc->getText("adminTipoPrestamos_newAdded"); ?><br><br>
